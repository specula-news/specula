...
# --- UNSPLASH API SÖK (med din Access Key) ---
def search_unsplash_image(query):
    try:
        ...
        if not query:
            return None
        url = f"https://api.unsplash.com/search/photos?query={requests.utils.quote(query)}&per_page=1&orientation=landscape"
        headers = {"Authorization": "Client-ID NjC7cLC58hqhxaG7WQi4Ro2ghNdV5ZVI2VxXBQPidW8"}  # DIN ACCESS KEY
        response = requests.get(url, headers=headers, timeout=TIMEOUT_SECONDS)
        if response.status_code == 200:
            data = response.json()
            if data['results']:
                img_url = data['results'][0]['urls']['regular']
                if not image_manager.is_url_used(img_url):
                    image_manager.mark_as_used(img_url)
                    return img_url
...
def get_best_image(entry, category, article_url, source_name, title):
    ...
    # 1. Om källan är svartlistad -> Försök Unsplash sök baserat på titel
    if any(banned in source_lower for banned in BANNED_SOURCES):
        unsplash_img = search_unsplash_image(title)
        if unsplash_img:
            return unsplash_img
        else:
    ...
    # 4. Om ingen bild hittades, försök Unsplash
    unsplash_img = search_unsplash_image(title)
    if unsplash_img:
        return unsplash_img

    # 5. Sista utväg för alla: Kortleken
    return image_manager.get_random_fallback()
...
def generate_site():
    print("Startar SPECULA Generator v10.10.0 (Unsplash Integrated)...")
    all_articles = []
    seen_titles = set()
    ...
    for url, category in RSS_SOURCES:
        try:
            ...
            for entry in feed.entries[:MAX_ARTICLES_PER_SOURCE]:
                ...
                final_image = get_best_image(entry, category, entry.link, source_name, title)
